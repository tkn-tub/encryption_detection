...
void runs_plugin(size_t *payloadsLength, unsigned char **strArray, uint8_t threshold, std::vector<double>& stRepOfPvals)
{
    double pValues[24];
    double stdev, mean;
    ...
    for (uint8_t i = 0; i < threshold; i++)
    {
        ...
        Runs4(n, (*(strArray + i)), p_value);
        pValues[i] = p_value;
        // printf("I AM INSIDE PLUG IN runs_plugin");
            // printf("  RUNS PVALUE       %.6lf,\t\t\n", pValues[i]);
    }
