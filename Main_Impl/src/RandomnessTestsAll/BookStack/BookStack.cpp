...
void book_stack_plugin(RInside &Rqm, size_t *payloadsLength, const char **listOfPayloads, uint8_t threshold, std::vector<double> &stRepOfPvals)
{
    double pValues[24];
    double stdev, mean;
    ...
    for (uint8_t i = 0; i < threshold; i++)
    {
        ...
        if (pln < 100)
            ...
            bookStackInR(Rqm, (*(listOfPayloads + i)), p_value);
        pValues[i] = p_value;
        // printf("I AM INSIDE PLUG IN bookStack_plugin");
        // printf("  BKSTK PVALUE       %.6lf,\t\t\n", pValues[i]);
    }
