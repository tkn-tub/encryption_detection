...
void serial_plugin(size_t *payloadsLength, unsigned char **strArray, uint8_t threshold, std::vector<double> &stRepOfPvals)
{
    double pValues[24];
    double stdev, mean;
    ...
    for (uint8_t i = 0; i < threshold; i++)
    {
        ...
        int m; //default is 16 in origial impl.
        if (pln < 16)
            m = 2;
        else if (pln < 128)
            m = 4;
        ...
        Serial4(m, n, (*(strArray + i)), p_value);
        pValues[i] = p_value;
        // printf("I AM INSIDE PLUG IN serial_plugin");
        // printf("SER PVALUE       %.6lf,\t\t\n", pValues[i]);
    }
