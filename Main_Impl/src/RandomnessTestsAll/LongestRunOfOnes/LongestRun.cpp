/* This plugin is for Longest Run Of Ones RT
input:
...
void longest_run_plugin(size_t *payloadsLength, unsigned char **strArray, uint8_t threshold, std::vector<double>& stRepOfPvals)
{
    double pValues[24];
    double stdev, mean;
    ...
    for (uint8_t i = 0; i < threshold; i++)
    {
        ...
        LongestRunOfOnes3(n, (*(strArray + i)), p_value);
        pValues[i] = p_value;
        // printf("I AM INSIDE PLUG IN longest_run_plugin");
            // printf("  LRun PVALUE       %.6lf,\t\t\n", pValues[i]);
    }
